...
<?PHP
	...
	if($cn)
	{
		...
		if($db)
		{
			$q=mysqli_query($cn,"Select * from client_service_details where S_P_Id='$sp'");
			...
			if($q)
			{
				while($row=mysqli_fetch_row($q))
				{
					echo "<tr><td>".$row[6]."</td>";
						...
						//$q4=mysql_query("select Client_Email_Id from client_details where S_P_Id='$sp'");
						//$email=mysql_fetch_row($q4);
