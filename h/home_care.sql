...
INSERT INTO `admin` (`rowid`, `Name`, `Contact`, `Address`, `A_Email_Id`, `A_Password`, `City`) VALUES
(1, 'Omkar Pandya', '8980611411', 'Bolbala Marg, Rajkot', 'omkar.p@ahduni.edu.in', 'AU2040180', 'Rajkot'),
(2, 'Kinal Kagathara', '7990400277', '150ft ring road,Rajkot', 'kinal.k@ahduni.edu.in', 'AU2040240', 'Rajkot');
...
('Kanvi Patel', 'kanvi.p@ahduni.edu.in', '7984771151', 'Vijapur', 'T.B.Road, Vijapur', 'AU2040235', 0, 235);
...
CREATE TABLE IF NOT EXISTS `client_service_details` (
  ...
  KEY `FK1` (`Service_Id`) USING BTREE,
  KEY `FK2` (`S_P_Id`),
  KEY `FK3` (`Sub_Service_Id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

INSERT INTO `client_service_details` (`Record_No`, `Client_Email_Id`, `Service_Id`, `Sub_Service_Id`, `S_P_Id`, `C_Address`, `C_Date`, `P_Date`, `Provided`, `Payment`) VALUES
(1, 'akshat.s4@gmail.com', 'S001', 'S001_03', 'SP001', 'Palavasana Chowkdi, Mehsana', '2022-04-18', '2022-04-28', 1, 1);
...
INSERT INTO `contract_details` (`C_Id`, `C_Email_Id`, `Service_Id`, `S_P_Id`, `S_C_Plan`, `C_Start_Date`, `C_End_Date`, `Total_Cost`, `Payment`) VALUES
(1, 'akshat.s4@ahduni.edu.in', 'S001', 'SP001', '3 Month', '2022-04-18', '2022-07-18', 600, 1);
...
INSERT INTO `contract_record` (`Id`, `C_Id`, `C_Date`, `P_Date`, `Provided`) VALUES
(1, 1, '2022-04-18', '2022-04-18', 1),
(2, 1, '2022-04-18', '2022-04-18', 1);
...
CREATE TABLE IF NOT EXISTS `feedback_details` (
  ...
  `S_P_Id` varchar(10) NOT NULL,
  PRIMARY KEY (`F_Id`),
  KEY `Foreign key` (`Service_Id`),
  KEY `FK4` (`S_P_Id`),
  KEY `FK5` (`C_Email_Id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

INSERT INTO `feedback_details` (`F_Id`, `C_Email_Id`, `C_Feedback`, `C_F_Date`, `Service_Id`, `S_P_Id`) VALUES
(1, 'akshat.s4@ahduni.edu.in', 'Having good knowledge about regrigerator\r\nProvide tips to maintain refrigerator\r\nPunctual in time', '2022-04-18', 'S001', 'SP001');
...
--
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `FeedbackInsert`(IN `email` VARCHAR(40), IN `feedback` VARCHAR(500), IN `date` DATE, IN `S_id` VARCHAR(10), IN `S_P_id` VARCHAR(10))
INSERT INTO `feedback_details`(`C_Email_Id`, `C_Feedback`, `C_F_Date`, `Service_Id`, `S_P_Id`) VALUES (email,feedback,date,S_id,S_P_id)$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetUserByEmail`(IN `eid` VARCHAR(40))
BEGIN
select C_Name from client_details where C_Email_Id=eid;
END$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `allservice`()
BEGIN
select * from service;
END$$
DELIMITER ;

CREATE TRIGGER `phone` BEFORE INSERT ON `client_details`
 FOR EACH ROW BEGIN
    IF (LENGTH(NEW.C_Contact) != 10) THEN 
        SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Warning: Contact Number must be 10 digits.';
    END IF;
END
SET @p0='S001'; CALL `subservice`(@p0);
CALL `allservice`();
SET @p0='akshat.s4@ahduni.edu.in'; CALL `GetUserByEmail`(@p0);
...
ALTER TABLE `feedback_details`
  ...
  ADD CONSTRAINT `Foreign key` FOREIGN KEY (`Service_Id`) REFERENCES `service` (`Service_Id`) ON DELETE CASCADE ON UPDATE CASCADE;
